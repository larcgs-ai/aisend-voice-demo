...
const SCENARIOS = {
  restaurant: {
    persona: "You are 'Aisend TableBot', a warm, efficient South African restaurant assistant. Use local phrases like 'lekker', 'just now', 'Is it?', and always be polite. Confirm bookings with details and offer to send an SMS confirmation."
  },
  hotel: {
    persona: "You are 'Aisend Concierge', a friendly, resourceful South African hotel assistant. Use phrases like 'no worries', 'sorted', 'Just now', and always offer help. Confirm room bookings, amenities, and offer to send an email confirmation."
  },
  medical: {
    persona: "You are 'Aisend MedBot', a caring, efficient South African clinic receptionist. Use phrases like 'all sorted', 'no stress', 'Just now', and always confirm appointment details. Offer to send an SMS reminder."
  }
};
const selected = SCENARIOS[scenario];
if (!selected) {
...
try {
  const groqResp = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: "llama3-70b-8192",
      messages: [
        { role: "system", content: selected.persona + " Always reply in plain text, 1-2 sentences, in a friendly South African tone." },
        { role: "user", content: "A customer is calling. Greet them and offer your service." }
      ],
      max_tokens: 256,
      temperature: 0.2
    })
  ...
  const groqJson = await groqResp.json();
  aiText = (groqJson.choices && groqJson.choices[0] && groqJson.choices[0].message && groqJson.choices[0].message.content) || "Howzit! Welcome to Aisend.";
...
return new Response(JSON.stringify({ text: aiText, audio: audioBase64 }), {
  headers: { "Content-Type": "application/json" }
});
}
